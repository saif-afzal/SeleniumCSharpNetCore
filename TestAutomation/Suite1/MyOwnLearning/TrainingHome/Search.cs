...
namespace Selenium2.Meridian.Suite.MyOwnLearning.TrainingHome
{
    ...
    class Searchold : TestBase
    {
        string browserstr = string.Empty;
        public Searchold(string browser)
            : base(browser)
        {
        ...
        public  void a_Conduct_a_simple_search_for_a_course_content_item()
        {
            ...
            Assert.IsTrue(TrainingHomeobj.Click_Search(ExtractDataExcel.MasterDic_genralcourse["Title"] + browserstr + "simple_search"));
            SearchResultsobj.Content_Click();
            Assert.IsTrue(Detailsobj.Check_ContentHeading(ExtractDataExcel.MasterDic_genralcourse["Title"] + browserstr + "simple_search"));
        }
        [TearDown]
